...
export async function requestOtp(
  ...
  const [phoneCountResult] = await db.select({ count: sql<number>`count(*)::int` })
    ...
    .where(and(
      ...
      gt(otpAttempts.createdAt, oneHourAgo),
    ));

  if ((phoneCountResult?.count ?? 0) >= 5) {
  ...
  return { ok: true, expiresAt, ...(isDev ? { code } : {}) };
}
