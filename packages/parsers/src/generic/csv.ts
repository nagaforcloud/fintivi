import { parse } from 'csv-parse/sync';
import type { ParsedTransaction, ParserCandidate, CsvParseResult } from '../types';

const DATE_KEYS = ['date', 'transaction date', 'txn date', 'posted', 'posting date'];
const DESC_KEYS = ['description', 'narration', 'particulars', 'details', 'memo', 'payee'];
const AMOUNT_KEYS = ['amount', 'amt', 'value', 'sum'];
const DEBIT_KEYS = ['debit', 'withdrawal', 'debit amount'];
const CREDIT_KEYS = ['credit', 'deposit', 'credit amount'];

function findKey(header: string[], candidates: string[]): string | null {
  for (const c of candidates) {
    const found = header.find((h) => h.toLowerCase().trim() === c);
    if (found) return found;
  }
  return null;
}

function parseAmount(raw: string): number {
  const cleaned = raw.replace(/[,\s₹$€£]/g, '').replace(/\((.*)\)/, '-$1');
  const n = Number(cleaned);
  return Number.isFinite(n) ? n : NaN;
}

function toMinor(amount: number): number {
  return Math.round(amount * 100);
}

function hashFingerprint(date: string, description: string, amountMinor: number): string {
  const str = `${date}|${description.toLowerCase().trim()}|${amountMinor}`;
  let hash = 0;
  for (let i = 0; i < str.length; i++) {
    const chr = str.charCodeAt(i);
    hash = ((hash << 5) - hash) + chr;
    hash |= 0;
  }
  return hash.toString(16);
}

export function parseCsv(buffer: Buffer, filename: string): {
  rows: CsvParseResult[];
  candidates: ParserCandidate[];
  warnings: string[];
} {
  const records = parse(buffer, { columns: true, skip_empty_lines: true, trim: true }) as Record<string, string>[];
  const warnings: string[] = [];

  if (!records.length) {
    return { rows: [], candidates: [], warnings: ['empty CSV'] };
  }

  const header = Object.keys(records[0]!);
  const dateKey = findKey(header, DATE_KEYS);
  const descKey = findKey(header, DESC_KEYS);
  const amountKey = findKey(header, AMOUNT_KEYS);
  const debitKey = findKey(header, DEBIT_KEYS);
  const creditKey = findKey(header, CREDIT_KEYS);

  const foundKeys = [dateKey, descKey, amountKey || debitKey || creditKey].filter(Boolean).length;
  const totalKeyTypes = 3;
  const confidence = foundKeys / totalKeyTypes;

  if (!dateKey) {
    warnings.push('no date column found');
    return { rows: [], candidates: [makeCandidate('generic-csv', confidence, warnings)], warnings };
  }
  if (!descKey) {
    warnings.push('no description column found');
    return { rows: [], candidates: [makeCandidate('generic-csv', confidence, warnings)], warnings };
  }
  if (!amountKey && !debitKey && !creditKey) {
    warnings.push('no amount/debit/credit column found');
    return { rows: [], candidates: [makeCandidate('generic-csv', confidence, warnings)], warnings };
  }

  const rows: CsvParseResult[] = [];

  for (const r of records) {
    const date = r[dateKey!] ?? '';
    const description = r[descKey!] ?? '';
    let amount: number;

    if (amountKey) {
      amount = parseAmount(r[amountKey!] ?? '');
    } else {
      const debit = debitKey ? parseAmount(r[debitKey!] ?? '0') : 0;
      const credit = creditKey ? parseAmount(r[creditKey!] ?? '0') : 0;
      amount = (Number.isFinite(credit) ? credit : 0) - (Number.isFinite(debit) ? debit : 0);
    }

    if (!Number.isFinite(amount)) continue;

    rows.push({
      postedAt: normalizeDate(date),
      description,
      amountMinor: toMinor(amount),
      raw: r as Record<string, unknown>,
    });
  }

  return {
    rows,
    candidates: [makeCandidate('generic-csv', confidence >= 0.66 ? 0.85 : confidence, warnings)],
    warnings,
  };
}

function makeCandidate(parserId: string, confidence: number, warnings: string[]): ParserCandidate {
  return { parserId, market: 'global', confidence, warnings };
}

function normalizeDate(dateStr: string): string {
  const trimmed = dateStr.trim();
  const parts = trimmed.split(/[/\-.]/);
  if (parts.length === 3) {
    if (parts[2]!.length === 4) {
      if (parts[0]!.length === 4) return `${parts[0]}-${parts[1]!.padStart(2, '0')}-${parts[2]!.padStart(2, '0')}`;
      if (parts[2]!.length === 4) return `${parts[2]}-${parts[0]!.padStart(2, '0')}-${parts[1]!.padStart(2, '0')}`;
    }
  }
  return trimmed;
}

export function toTransaction(row: CsvParseResult, currency: string = 'USD'): ParsedTransaction {
  return {
    postedAt: row.postedAt,
    description: row.description,
    amountMinor: row.amountMinor,
    currency,
    externalFingerprint: hashFingerprint(row.postedAt, row.description, row.amountMinor),
    raw: row.raw,
  };
}
